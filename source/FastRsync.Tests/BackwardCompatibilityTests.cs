...
public class BackwardCompatibilityTests
{
    ...
    public async Task ApplyOldPatch(string deltaFileName)
    {
        ...
        using var baseFileStream = new FileStream(baseFileName, FileMode.Open);
        using var deltaFileStream = new FileStream(deltaFileName, FileMode.Open);
        ...
        // Assert
        var newFileExpectedData = File.ReadAllBytes(baseFile2Name);
    ...
    public async Task BuildPatchFromOldSignature(string signatureFileName)
    {
        ...
        using var signatureFileStream = new FileStream(signatureFileName, FileMode.Open);

        var progressReporter = Substitute.For<IProgress<ProgressReport>>();
        var patchedDataStream = new MemoryStream();

        // Act
        using (var baseFile2Stream = new FileStream(baseFile2Name, FileMode.Open))
        {
